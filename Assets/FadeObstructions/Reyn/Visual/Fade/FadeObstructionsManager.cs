...
public class FadeObstructionsManager : MonoBehaviour
{
    ...
    [Range(0,1)]public float FinalAlpha = 0.1f;
    ...
    public GameObject ShouldBeVisibleObjects;
    ...
    public void RegisterShouldBeVisible(GameObject shouldBeVisible)
    {
        ...
        ShouldBeVisibleObjects = shouldBeVisible;
    ...
    void Update()
    {
        ...
        if (ShouldBeVisibleObjects == null)
            ShouldBeVisibleObjects = nullObject;

        if (ShouldBeVisibleObjects != nullObject) {
            ...
            //{
            Vector3 diffVector = ShouldBeVisibleObjects.transform.position - Camera.transform.position;
            ...
            foreach (RaycastHit hit in hits) {
                // Get the collider
                var c = hit.collider;
                var parentC = c.transform.parent;
        ...
        foreach (var go in objectsInWay)
        {
            ...
            if (hiddenObject == null)
            {
               // Get the object fade options
                ...
                go.gameObject.AddComponent<NotifyFadeSystem>();
            }
        }
